...
// Fix: Augment global AIStudio interface and ensure window.aistudio matches existing definition
declare global {
  interface AIStudio {
...
const App: React.FC = () => {
  ...
  const openKeySelection = async () => {
    try {
      // Fix: Use the established interface for key selection
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        if (err.message === "KEY_ERROR") {
          setErrorMessage("Parece que há um problema com a chave de acesso da API (Erro 500/401).");
          setStep(AppStep.UPLOAD);
        } else {
          setErrorMessage(err.message || "Ocorreu um erro inesperado ao analisar o contrato.");
          setStep(AppStep.UPLOAD);
        }
      } finally {
        setLoading(false);
  ...
  const handleStartNegotiation = async (stage: NegotiationStage) => {
    ...
    try {
      ...
      if (err.message === "KEY_ERROR") {
        setErrorMessage("Erro de autenticação ao gerar documento.");
      } else {
    ...
    <div className="min-h-screen bg-[#0a0f1a] flex flex-col items-center justify-start overflow-x-hidden">
      ...
      <div className="w-full bg-white rounded-t-[3rem] px-6 py-10 flex-grow shadow-[-10px_0_30px_rgba(0,0,0,0.4)]">
        <div className="max-w-md mx-auto">
          ...
          >
            {loading ? <Loader2 className="animate-spin text-amber-500" size={28} /> : "Iniciar Diagnóstico do Contrato"}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="w-full px-6 py-10 flex flex-col">
      <div className="text-center mb-10">
        ...
        <div className="flex flex-col gap-4 mb-10">
          ...
          >
            Analisar Contrato Agora <ArrowRight size={22} />
          </button>
          <a href={BCB_CALCULATOR_URL} target="_blank" rel="noopener noreferrer" className="w-full bg-slate-50 border border-slate-200 text-slate-700 py-4 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-slate-100 transition-all">
            <Building2 size={18} className="text-amber-600" /> Calculadora do Cidadão (BCB)
          </a>
        </div>
      </div>
    </div>
        ...
        <div className="mb-6 p-5 bg-red-50 border border-red-100 rounded-3xl flex flex-col gap-4">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-600 mt-1">Isso ocorre geralmente por falha na autenticação da IA ou limite de uso.</p>
            </div>
          </div>
          <button 
            onClick={openKeySelection}
            className="w-full bg-red-600 text-white py-3 rounded-xl font-black text-xs flex items-center justify-center gap-2 shadow-lg active:scale-[0.98]"
          >
            <Key size={16} /> Configurar Acesso (Selecionar Chave)
          </button>
        </div>
      )}
      <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl border border-slate-100 text-center relative overflow-hidden">
        ...
        <label className="block w-full cursor-pointer">
          <div className="border-2 border-dashed border-slate-200 rounded-3xl py-12 px-6 bg-slate-50/50 hover:bg-amber-50 hover:border-amber-500 transition-all">
            <span className="block text-base font-bold text-slate-700 mb-2 text-center">Selecionar Contrato (PDF)</span>
  ...
  const renderAnalysis = () => (
    <div className="px-6 py-32 text-center">
      ...
      <p className="text-sm text-slate-500 px-8">Identificando taxas e calculando a projeção de economia real.</p>
    </div>
  );
      ...
      <div className="px-6 py-8 flex flex-col gap-6 pb-32">
        <div className="flex items-center justify-between gap-4 mb-2">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <h1 className="text-xl font-black text-slate-900 tracking-tight">Resultado do Diagnóstico</h1>
                 <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">{session.fileName}</p>
              </div>
           </div>
           <a href={BCB_CALCULATOR_URL} target="_blank" rel="noopener noreferrer" className="p-3 bg-white border border-slate-200 rounded-xl text-slate-600 shadow-sm"><Calculator size={20} /></a>
        </div>

        <div className={`p-6 rounded-[2rem] border overflow-hidden ${analysis.status === 'critical' ? 'bg-red-50 border-red-100' : 'bg-amber-50 border-amber-100'}`}>
          <div className="flex items-center gap-2 mb-6">
            <AlertTriangle size={20} className={analysis.status === 'critical' ? 'text-red-600' : 'text-amber-600'} />
            <span className="font-black text-[10px] uppercase tracking-widest text-slate-700">Abusividades Identificadas</span>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 mb-8">
            ...
            <div className="bg-white/60 backdrop-blur-sm p-4 rounded-2xl border border-white flex flex-col gap-3 shadow-sm">
              <div className="flex justify-between items-end border-b border-slate-100 pb-2">
                <div>
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Parcela Atual</p>
                  <p className="text-base font-bold text-slate-600 line-through decoration-red-500/50 decoration-2">R$ {analysis.currentInstallmentValue.toFixed(2)}</p>
                </div>
                <div className="text-right">
                  <p className="text-[8px] font-black text-amber-500 uppercase tracking-widest mb-0.5">Distorção Mensal</p>
                  <p className="text-base font-black text-amber-600">- R$ {analysis.estimatedMonthlySavings.toFixed(2)}</p>
                </div>
              </div>
              <div className="py-2">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-0.5 text-center">Sua Nova Parcela Justa</p>
                <p className="text-4xl font-black text-[#0a0f1a] tracking-tighter text-center">R$ {projectedFairInstallment.toFixed(2)}</p>
              </div>
            </div>
          </div>
          
          <div className="bg-white p-5 rounded-2xl shadow-inner border border-slate-100">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Valor Cobrado vs. Justo</p>
        ...
        <div className="space-y-3">
          ...
          {analysis.irregularities.map((irr, idx) => (
            <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <p className="text-xs text-slate-500 mb-3 leading-relaxed">{irr.description}</p>
              <div className="text-[9px] font-black text-amber-600 bg-amber-50 px-2 py-1 rounded-lg inline-block">Ref: {irr.legalReference}</div>
            </div>
          ))}
        </div>

        <div className="fixed bottom-0 left-0 right-0 p-5 bg-white/90 backdrop-blur-md border-t border-slate-100 flex gap-3 z-40">
          <button onClick={downloadTechnicalReportPDF} className="flex-1 bg-white border border-slate-200 text-slate-700 p-4 rounded-xl font-bold text-xs">Laudo Técnico</button>
          <button onClick={() => handleStartNegotiation('sac')} className="flex-[2] bg-amber-500 text-[#0a0f1a] p-4 rounded-xl font-black text-xs flex items-center justify-center gap-2 shadow-xl active:scale-[0.98]">Acessar Negociação <ChevronRight size={16} /></button>
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="px-6 py-8 flex flex-col gap-6 pb-32">
        ...
        <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100">
                ...
                <div className="grid grid-cols-1 gap-3">
                  ...
                  {stage === 'reclame-aqui' && selectedBank.reclameAquiUrl && (
                    <a href={selectedBank.reclameAquiUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 bg-[#e60000] p-4 rounded-xl font-black text-white shadow-lg active:scale-[0.98]">
                      <ExternalLink size={18} /> Ir para Reclamação Direta
                    </a>
                  )}
                </div>
              )}
          ...
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 mb-6 font-mono text-[9px] leading-relaxed whitespace-pre-wrap max-h-[250px] overflow-y-auto text-slate-600 border-l-4 border-l-amber-500">
            {documentContent}
          </div>

          <div className="flex flex-col gap-3">
            <button onClick={() => { navigator.clipboard.writeText(documentContent); alert("Conteúdo copiado!"); }} className="w-full bg-[#0a0f1a] text-white py-4 rounded-xl font-black text-base shadow-lg">Copiar Minuta de Defesa</button>
            <button onClick={downloadPDF} className="w-full bg-slate-50 text-slate-900 py-3 rounded-xl font-bold text-xs border border-slate-200 flex items-center justify-center gap-2"><Download size={16} /> Salvar PDF</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col">
      <nav className="bg-white/90 backdrop-blur-xl border-b border-slate-100 sticky top-0 z-50">
        <div className="px-6 h-16 flex justify-between items-center">
          <div className="cursor-pointer" onClick={() => setStep(AppStep.LANDING)}><Logo size="sm" /></div>
